...
function call_active_action(session_id) {
  ...
  return db.child('/sessions').child(session_id || "missigno").once('value').then(function(snapshot) {
    ...
    if(rounds == null) {
      ...
      return build_action_json("active_turn", {
    ...
    if(__.isEmpty(json_response.action.data)) {
      ...
      if(req.params.thing_id == json_response.action.data.hero_id) {
        ...
        if(json_response.action.data.round_id) {
          ...
          var roundref = db.child('sessions').child(req.params.session_id).child("rounds").push();
        ...
        res.send(build_action_json("action_saved", action_object));

      } else {
        res.send(build_error_json(666, "Não vem hackear aqui não!"));
      }
    }
